...
// Helper: process a single MQTT packet
void process_packet(const uint8_t *buf, size_t len) {
  struct mg_mqtt_message mm;
  memset(&mm, 0, sizeof(mm));
  mm.dgram.ptr = (char *)buf;
  mm.dgram.len = len;
  mm.cmd = (buf[0] >> 4) & 0x0F;
  mm.qos = (buf[0] >> 1) & 0x03;
  fn(&dummy_conn, MG_EV_MQTT_CMD, &mm, NULL);
}

int main() {
  // Prepare dummy connection
  memset(&dummy_conn, 0, sizeof(dummy_conn));
  dummy_conn.fd = &dummy_fd;

  // Read all input from stdin
  uint8_t buf[65536];
  size_t total = fread(buf, 1, sizeof(buf), stdin);
  size_t pos = 0;

  // Each packet is length-prefixed: [2 bytes len][data...]
  while (pos + 2 <= total) {
    uint16_t pkt_len = (buf[pos] << 8) | buf[pos+1];
    pos += 2;
    if (pos + pkt_len > total) break;
    process_packet(buf + pos, pkt_len);
    pos += pkt_len;
  }
