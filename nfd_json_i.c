// --- Auto-generated: Normalized Fuzz Driver ---
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int NFD(const uint8_t *input, size_t input_len) {
    ...
    __AFL_INIT();
#endif
    // Process the entire input buffer as one giant string, null-terminated
    if (input_len == 0) return 0;

    // Cap at BUFFER_SIZE-1 to avoid overflow, just like before
    size_t to_copy = input_len < (BUFFER_SIZE - 1) ? input_len : (BUFFER_SIZE - 1);
    char buf[BUFFER_SIZE];
    memcpy(buf, input, to_copy);
    buf[to_copy] = '\0';

    // Split and process each command as before
