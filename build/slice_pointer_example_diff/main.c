...
typedef struct Int32Slice_DecodeCtx {
    int *data;
    size_t count;
    size_t capacity;
} Int32Slice_DecodeCtx;

static bool decode_Int32Slice_data(pb_istream_t *stream, const pb_field_t *field, void **arg) {
    (void)field;
    Int32Slice_DecodeCtx *ctx = (Int32Slice_DecodeCtx*)(*arg);
    while (stream->bytes_left > 0) {
        uint64_t tmp = 0;
        if (!pb_decode_varint(stream, &tmp)) {
            return false;
        }
        int value = (int)((int32_t)tmp);
        if (ctx->count >= ctx->capacity) {
            size_t new_capacity = ctx->capacity ? ctx->capacity * 2 : 8;
            int *new_data = (int*)realloc(ctx->data, new_capacity * sizeof(int));
            if (!new_data) {
                return false;
            }
            ctx->data = new_data;
...
extern int sum_slice(const int * values, int count);
...
int pin_wrapper_entry(const uint8_t *data, size_t len) {
    ...
    Int32Slice_DecodeCtx values_ctx = {NULL, 0, 0};
    input.values.data.funcs.decode = decode_Int32Slice_data;
    input.values.data.arg = &values_ctx;

    pb_istream_t stream = pb_istream_from_buffer(data, len);
    if (!pb_decode(&stream, Input_fields, &input)) {
        if (values_ctx.data) { free(values_ctx.data); }
        return 1;
    }
    int32_t values_length_raw = input.values.length;
    size_t values_len = values_ctx.count;
    int * values_storage = values_ctx.data;
    values_ctx.data = NULL;
    const int * values_ptr = NULL;
    if (values_storage && values_len > 0) {
        values_ptr = (const int *)values_storage;
    }
    input.has_values = (values_len > 0);
    input.values.length = (int32_t)values_len;
...
emi_finish:
    if (values_storage) { free(values_storage); }
