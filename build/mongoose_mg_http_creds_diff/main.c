...
bool decode_user(pb_istream_t *stream, const pb_field_t *field, void **arg) {
...
bool decode_pass(pb_istream_t *stream, const pb_field_t *field, void **arg) {
    char *buffer = (char *)(*arg);
    size_t len = stream->bytes_left;
    if (len >= 128) {
        if (!pb_read(stream, (pb_byte_t*)buffer, 128 - 1)) {
...
extern void mg_http_creds(struct mg_http_message * hm, char * user, size_t userlen, char * pass, size_t passlen);
...
int pin_wrapper_entry(const uint8_t *data, size_t len) {
    ...
    user_buf[0] = '\0';
    input.user.arg = user_buf;
    input.user.funcs.decode = &decode_user;

    pass_buf[0] = '\0';
    input.pass.arg = pass_buf;
    input.pass.funcs.decode = &decode_pass;

    pb_istream_t stream = pb_istream_from_buffer(data, len);
    if (!pb_decode(&stream, Input_fields, &input)) {
        return 1;
    }
    mg_http_message hm_storage = {0};
    struct mg_http_message * hm_ptr = (struct mg_http_message *)&hm_storage;
    if (input.hm.present) {
        hm_storage = input.hm.value;
    }

    mg_http_creds(hm_ptr, user_buf, input.userlen, pass_buf, input.passlen);
    goto emi_finish;
