...
extern "C" void mg_http_creds(struct mg_http_message * hm, char * user, size_t userlen, char * pass, size_t passlen);
...
int pin_reference_entry(const uint8_t *data, size_t len) {
    ...
    mg_http_creds(hm_ptr, msg.user().c_str(), msg.userlen(), msg.pass().c_str(), msg.passlen());
    return 0;
}
