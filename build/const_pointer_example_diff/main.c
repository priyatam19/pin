...
extern double calculate_distance(const struct Point * p1, const struct Point * p2);
...
int pin_wrapper_entry(const uint8_t *data, size_t len) {
    ...
    Point p1_storage = {0};
    const struct Point * p1_ptr = NULL;
    if (input.p1.present) {
        p1_storage = input.p1.value;
        p1_ptr = (const struct Point *)&p1_storage;
    }
    Point p2_storage = {0};
    const struct Point * p2_ptr = NULL;
    if (input.p2.present) {
        p2_storage = input.p2.value;
        p2_ptr = (const struct Point *)&p2_storage;
    }
