...
extern int mg_http_get_request_len(const unsigned char * buf, size_t buf_len);
...
int pin_wrapper_entry(const uint8_t *data, size_t len) {
    ...
    if (input.buf_len != (size_t)buf_len_len) {
        emi_reason = PIN_EMI_REASON_LENGTH_MISMATCH;
        emi_detail = "buf_len";
        goto emi_reject;
    }
    mg_http_get_request_len(buf_buf, (size_t)buf_len_len);
    goto emi_finish;
