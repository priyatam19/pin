...
extern int mg_timer_expired(uint64_t * t, uint64_t prd, uint64_t now);
...
int pin_wrapper_entry(const uint8_t *data, size_t len) {
    ...
    uint64_t t_storage = 0;
    uint64_t * t_ptr = &t_storage;
    if (input.t.has_value) {
        t_storage = (uint64_t)(input.t.value);
    }

    mg_timer_expired(t_ptr, input.prd, input.now);
    goto emi_finish;
