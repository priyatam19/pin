...
extern "C" int mg_timer_expired(uint64_t * t, uint64_t prd, uint64_t now);
...
int pin_reference_entry(const uint8_t *data, size_t len) {
    ...
    const auto& t_msg = msg.t();
    uint64_t t_storage = 0;
    uint64_t * t_ptr = nullptr;
    if (t_msg.has_value()) {
        t_storage = static_cast<uint64_t>(t_msg.value());
        t_ptr = &t_storage;
    }
    mg_timer_expired(t_ptr, msg.prd(), msg.now());
    return 0;
}
