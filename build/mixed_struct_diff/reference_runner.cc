...
extern "C" int analyze_sensor(int id, double reading, int flag0, int flag1, int flag2, const char * note);
...
int pin_reference_entry(const uint8_t *data, size_t len) {
    ...
    analyze_sensor(msg.id(), msg.reading(), msg.flag0(), msg.flag1(), msg.flag2(), msg.note().c_str());
    return 0;
}
