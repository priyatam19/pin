...
bool decode_digest(pb_istream_t *stream, const pb_field_t *field, void **arg) {
...
extern void mg_sha1_final(unsigned char[20] digest, mg_sha1_ctx * context);
...
int pin_wrapper_entry(const uint8_t *data, size_t len) {
    ...
    char digest_buf[128] = {0};


    digest_buf[0] = '\0';
    input.digest.arg = digest_buf;
    input.digest.funcs.decode = &decode_digest;
    ...
    mg_sha1_final(digest_buf, input.context);
    goto emi_finish;
