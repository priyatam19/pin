...
bool decode_pkt_primary(pb_istream_t *stream, const pb_field_t *field, void **arg) {
...
bool decode_pkt_secondary(pb_istream_t *stream, const pb_field_t *field, void **arg) {
    char *buffer = (char *)(*arg);
    size_t len = stream->bytes_left;
    if (len >= 128) {
        if (!pb_read(stream, (pb_byte_t*)buffer, 128 - 1)) {
...
extern int inspect_packet(struct NotePacket pkt);
...
int pin_wrapper_entry(const uint8_t *data, size_t len) {
    ...
    char pkt_primary_buf[128] = {0};
    char pkt_secondary_buf[128] = {0};


    pkt_primary_buf[0] = '\0';
    input.pkt.primary.arg = pkt_primary_buf;
    input.pkt.primary.funcs.decode = &decode_pkt_primary;

    pkt_secondary_buf[0] = '\0';
    input.pkt.secondary.arg = pkt_secondary_buf;
    input.pkt.secondary.funcs.decode = &decode_pkt_secondary;
