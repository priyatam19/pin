...
bool decode_body_ptr(pb_istream_t *stream, const pb_field_t *field, void **arg) {
...
extern int mg_http_next_multipart(struct mg_str body, size_t ofs, struct mg_http_part * part);
...
int pin_wrapper_entry(const uint8_t *data, size_t len) {
    ...
    char body_ptr_buf[128] = {0};


    body_ptr_buf[0] = '\0';
    input.body.ptr.arg = body_ptr_buf;
    input.body.ptr.funcs.decode = &decode_body_ptr;

    pb_istream_t stream = pb_istream_from_buffer(data, len);
    if (!pb_decode(&stream, Input_fields, &input)) {
        return 1;
    }
    mg_http_part part_storage = {0};
    struct mg_http_part * part_ptr = (struct mg_http_part *)&part_storage;
    if (input.part.present) {
        part_storage = input.part.value;
    }

    mg_http_next_multipart(input.body, input.ofs, part_ptr);
    goto emi_finish;
