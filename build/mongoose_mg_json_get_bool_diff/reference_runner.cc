...
extern "C" int mg_json_get_bool(struct mg_str json, const char * path, int * v);
...
int pin_reference_entry(const uint8_t *data, size_t len) {
    ...
    const auto& v_msg = msg.v();
    int v_storage = 0;
    int * v_ptr = nullptr;
    if (v_msg.has_value()) {
        v_storage = static_cast<int>(v_msg.value());
        v_ptr = &v_storage;
    }
    mg_json_get_bool(msg.json(), msg.path().c_str(), v_ptr);
    return 0;
}
