...
extern "C" void mg_mgr_poll(struct mg_mgr * mgr, int ms);
...
int pin_reference_entry(const uint8_t *data, size_t len) {
    ...
    const auto& mgr_msg = msg.mgr();
    mg_mgr mgr_storage = {};
    struct mg_mgr * mgr_ptr = nullptr;
    if (mgr_msg.has_value()) {
        mgr_storage = mgr_msg.value();
        mgr_ptr = &mgr_storage;
    }
    mg_mgr_poll(mgr_ptr, msg.ms());
    return 0;
}
