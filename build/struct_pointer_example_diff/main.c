...
extern double read_sensor_value(const struct Sensor * sensor);
...
int pin_wrapper_entry(const uint8_t *data, size_t len) {
    ...
    Sensor sensor_storage = {0};
    const struct Sensor * sensor_ptr = NULL;
    if (input.sensor.present) {
        sensor_storage = input.sensor.value;
        sensor_ptr = (const struct Sensor *)&sensor_storage;
    }
