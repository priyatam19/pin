...
extern "C" double read_sensor_value(const struct Sensor * sensor);
...
int pin_reference_entry(const uint8_t *data, size_t len) {
    ...
    const auto& sensor_msg = msg.sensor();
    Sensor sensor_storage = {};
    const struct Sensor * sensor_ptr = nullptr;
    if (sensor_msg.has_value()) {
        sensor_storage = sensor_msg.value();
        sensor_ptr = &sensor_storage;
    }
    read_sensor_value(sensor_ptr);
    return 0;
}
