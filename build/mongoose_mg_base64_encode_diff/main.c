...
bool decode_p(pb_istream_t *stream, const pb_field_t *field, void **arg) {
...
bool decode_to(pb_istream_t *stream, const pb_field_t *field, void **arg) {
    char *buffer = (char *)(*arg);
    size_t len = stream->bytes_left;
    if (len >= 128) {
        if (!pb_read(stream, (pb_byte_t*)buffer, 128 - 1)) {
...
extern int mg_base64_encode(const unsigned char * p, int n, char * to);
...
int pin_wrapper_entry(const uint8_t *data, size_t len) {
    ...
    p_buf[0] = '\0';
    input.p.arg = p_buf;
    input.p.funcs.decode = &decode_p;

    to_buf[0] = '\0';
    input.to.arg = to_buf;
    input.to.funcs.decode = &decode_to;
    ...
    if (input.n != (int)n_len) {
        emi_reason = PIN_EMI_REASON_LENGTH_MISMATCH;
        emi_detail = "n";
        goto emi_reject;
    }
    mg_base64_encode(p_buf, (int)n_len, to_buf);
    goto emi_finish;
