...
extern int process_file(int * fts, int * ent);
...
int pin_wrapper_entry(const uint8_t *data, size_t len) {
    ...
    int fts_storage = 0;
    int * fts_ptr = &fts_storage;
    if (input.fts.has_value) {
        fts_storage = (int)(input.fts.value);
    }
    int ent_storage = 0;
    int * ent_ptr = &ent_storage;
    if (input.ent.has_value) {
        ent_storage = (int)(input.ent.value);
    }
