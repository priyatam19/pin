...
extern int mg_json_get(struct mg_str json, const char * path, int * toklen);
...
int pin_wrapper_entry(const uint8_t *data, size_t len) {
    ...
    int * toklen_ptr = &toklen_storage;
    if (input.toklen.has_value) {
        toklen_storage = (int)(input.toklen.value);
    }

    mg_json_get(input.json, path_buf, toklen_ptr);
    goto emi_finish;
