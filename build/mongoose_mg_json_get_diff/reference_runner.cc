...
extern "C" int mg_json_get(struct mg_str json, const char * path, int * toklen);
...
int pin_reference_entry(const uint8_t *data, size_t len) {
    ...
    const auto& toklen_msg = msg.toklen();
    int toklen_storage = 0;
    int * toklen_ptr = nullptr;
    if (toklen_msg.has_value()) {
        toklen_storage = static_cast<int>(toklen_msg.value());
        toklen_ptr = &toklen_storage;
    }
    mg_json_get(msg.json(), msg.path().c_str(), toklen_ptr);
    return 0;
}
