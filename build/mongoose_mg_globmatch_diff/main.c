...
bool decode_s1(pb_istream_t *stream, const pb_field_t *field, void **arg) {
...
bool decode_s2(pb_istream_t *stream, const pb_field_t *field, void **arg) {
    char *buffer = (char *)(*arg);
    size_t len = stream->bytes_left;
    if (len >= 128) {
        if (!pb_read(stream, (pb_byte_t*)buffer, 128 - 1)) {
...
extern int mg_globmatch(const char * s1, size_t n1, const char * s2, size_t n2);
...
int pin_wrapper_entry(const uint8_t *data, size_t len) {
    ...
    s1_buf[0] = '\0';
    input.s1.arg = s1_buf;
    input.s1.funcs.decode = &decode_s1;

    s2_buf[0] = '\0';
    input.s2.arg = s2_buf;
    input.s2.funcs.decode = &decode_s2;
    ...
    mg_globmatch(s1_buf, input.n1, s2_buf, input.n2);
    goto emi_finish;
