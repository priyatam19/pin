...
extern "C" int process_generic(void * data, int data_type);
...
int pin_reference_entry(const uint8_t *data, size_t len) {
    ...
    const auto& data_msg = msg.data();
    void data_storage = 0;
    void * data_ptr = nullptr;
    if (data_msg.has_value()) {
        data_storage = static_cast<void>(data_msg.value());
        data_ptr = &data_storage;
    }
    process_generic(data_ptr, msg.data_type());
    return 0;
}
