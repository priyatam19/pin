...
__attribute__((weak)) cJSON * pin_acquire_handle_array(void) { return NULL; }
...
extern cJSON * test_cJSON_GetArrayItem(cJSON * array, int index);
...
int pin_wrapper_entry(const uint8_t *data, size_t len) {
    ...
    test_cJSON_GetArrayItem(pin_acquire_handle_array(), input.index);
    goto emi_finish;
