...
bool decode_suffix(pb_istream_t *stream, const pb_field_t *field, void **arg) {
    char *buffer = (char *)(*arg);
    size_t len = stream->bytes_left;
    if (len >= 128) {
        if (!pb_read(stream, (pb_byte_t*)buffer, 128 - 1)) {
...
extern void remove_suffix(char * name, const char * suffix);
...
int pin_wrapper_entry(const uint8_t *data, size_t len) {
    ...
    suffix_buf[0] = '\0';
    input.suffix.arg = suffix_buf;
    input.suffix.funcs.decode = &decode_suffix;
    ...
    remove_suffix(name_buf, suffix_buf);
    goto emi_finish;
