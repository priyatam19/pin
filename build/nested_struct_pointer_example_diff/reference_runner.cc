...
extern "C" int update_person(struct Person * person, int new_zip);
...
int pin_reference_entry(const uint8_t *data, size_t len) {
    ...
    const auto& person_msg = msg.person();
    Person person_storage = {};
    struct Person * person_ptr = nullptr;
    if (person_msg.has_value()) {
        person_storage = person_msg.value();
        person_ptr = &person_storage;
    }
    update_person(person_ptr, msg.new_zip());
    return 0;
}
