...
extern int update_person(struct Person * person, int new_zip);
...
int pin_wrapper_entry(const uint8_t *data, size_t len) {
    ...
    Person person_storage = {0};
    struct Person * person_ptr = NULL;
    if (input.person.present) {
        person_storage = input.person.value;
        person_ptr = (struct Person *)&person_storage;
    }

    update_person(person_ptr, input.new_zip);
    goto emi_finish;
