...
bool decode_file(pb_istream_t *stream, const pb_field_t *field, void **arg) {
...
bool decode_fname(pb_istream_t *stream, const pb_field_t *field, void **arg) {
    char *buffer = (char *)(*arg);
    size_t len = stream->bytes_left;
    if (len >= 128) {
        if (!pb_read(stream, (pb_byte_t*)buffer, 128 - 1)) {
...
extern int mg_log_prefix(int level, const char * file, int line, const char * fname);
...
int pin_wrapper_entry(const uint8_t *data, size_t len) {
    ...
    char file_buf[128] = {0};
    char fname_buf[128] = {0};


    file_buf[0] = '\0';
    input.file.arg = file_buf;
    input.file.funcs.decode = &decode_file;

    fname_buf[0] = '\0';
    input.fname.arg = fname_buf;
    input.fname.funcs.decode = &decode_fname;
    ...
    mg_log_prefix(input.level, file_buf, input.line, fname_buf);
    goto emi_finish;
