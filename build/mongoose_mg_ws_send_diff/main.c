...
extern int mg_ws_send(struct mg_connection * c, const void * buf, size_t len, int op);
...
int pin_wrapper_entry(const uint8_t *data, size_t len) {
    ...
    mg_connection c_storage = {0};
    struct mg_connection * c_ptr = (struct mg_connection *)&c_storage;
    if (input.c.present) {
        c_storage = input.c.value;
    }
    uint64_t buf_length_raw = input.buf.length;
    size_t buf_len = buf_ctx.count;
    void * buf_storage = buf_ctx.data;
    buf_ctx.data = NULL;
    const void * buf_ptr = NULL;
    if (buf_storage && buf_len > 0) {
        buf_ptr = (const void *)buf_storage;
    }
    ...
    mg_ws_send(c_ptr, buf_ptr, (size_t)buf_len, input.op);
    goto emi_finish;
