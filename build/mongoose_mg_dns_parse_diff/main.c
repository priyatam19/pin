...
extern int mg_dns_parse(const uint8_t * buf, size_t len, struct mg_dns_message * dm);
...
int pin_wrapper_entry(const uint8_t *data, size_t len) {
    ...
    mg_dns_message dm_storage = {0};
    struct mg_dns_message * dm_ptr = (struct mg_dns_message *)&dm_storage;
    if (input.dm.present) {
        dm_storage = input.dm.value;
    }
