...
extern "C" int mg_mqtt_next_topic(struct mg_mqtt_message * msg, struct mg_str * topic, uint8_t * qos, int pos);
...
int pin_reference_entry(const uint8_t *data, size_t len) {
    ...
    mg_mqtt_next_topic(msg_ptr, topic_ptr, qos_ptr, msg.pos());
    return 0;
}
